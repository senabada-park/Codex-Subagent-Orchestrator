...
Use only when the task genuinely includes release work:
...
- planner: `using-agent-skills` + `planning-and-task-breakdown`; add `context-engineering` when repo context selection matters
...
- fixer: `debugging-and-error-recovery` + `test-driven-development`; add one specialist skill only when the finding is clearly security, performance, or simplification related
