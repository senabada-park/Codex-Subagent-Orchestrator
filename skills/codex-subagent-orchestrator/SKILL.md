...
- read `../agent-skills-integration/agent-skill-routing.md` when you need planner, implementer, fixer, reviewer, or validator routing into the vendored `agent-skills` pack
...
- do not attach all vendored skills to every worker; route the minimum set that materially sharpens that worker's role through `../agent-skills-integration/agent-skill-routing.md`
...
Use the templates under `assets/run-templates/` when you want a consistent structure.

## Imported Discipline

The workspace vendors the full upstream `agent-skills` repository under `vendor/agent-skills/`.

Use it like this:

- planner: `using-agent-skills`, `context-engineering`, `spec-driven-development`, `planning-and-task-breakdown`
- implementer: `incremental-implementation`, `test-driven-development`, then task-shaped skills such as `frontend-ui-engineering`, `api-and-interface-design`, `ci-cd-and-automation`, `git-workflow-and-versioning`, or `deprecation-and-migration`
- fixer: `debugging-and-error-recovery`, `test-driven-development`, `incremental-implementation`
- reviewer: `code-review-and-quality`, then optional `code-simplification`, `security-and-hardening`, `performance-optimization`, `documentation-and-adrs`, or `git-workflow-and-versioning`
- validator: `test-driven-development`, `debugging-and-error-recovery`, then task-shaped verification skills such as `browser-testing-with-devtools`, `security-and-hardening`, `performance-optimization`, or `shipping-and-launch`
