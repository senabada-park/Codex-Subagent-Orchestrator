...
- read `../agent-skills-integration/agent-skill-routing.md` when you need phase-by-phase routing into the vendored `agent-skills` pack
...
- do not load all vendored skills at once; route only the minimum set needed for the current phase through `../agent-skills-integration/agent-skill-routing.md`
...
Use the templates under `assets/run-templates/` when you want a consistent starting point.
...
- `scan`: `using-agent-skills`, `context-engineering`, and optionally `idea-refine` or `spec-driven-development`
- `plan`: `planning-and-task-breakdown`, with `api-and-interface-design`, `documentation-and-adrs`, `ci-cd-and-automation`, `deprecation-and-migration`, or `shipping-and-launch` when the task surface needs them
- `implement`: `incremental-implementation` plus `test-driven-development`, then task-shaped skills such as `frontend-ui-engineering` or `api-and-interface-design`
- `verify`: `debugging-and-error-recovery`, with `browser-testing-with-devtools`, `security-and-hardening`, or `performance-optimization` when needed
- `review`: `code-review-and-quality`, then optional `code-simplification`, `security-and-hardening`, `performance-optimization`, `documentation-and-adrs`, or `git-workflow-and-versioning`
